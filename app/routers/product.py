...
def get_product_by_category(product_category: str, db: Session = Depends(get_db)):
    return get_product_category(product_category=product_category, db=db)


@router.get('/get_product_by_id/{product_id}', response_model=ProductResponse,
summary="Получение товара по его id", tags=["Products"])
def get_product_for_id(product_id: int, db: Session = Depends(get_db)):
